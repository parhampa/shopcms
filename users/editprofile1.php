...
$sql = "update users set `pass`='$pass',`mob`='$mob',`mob2`='$mob2',`tel`='$tel',`tel2`='$tel2',`title`='$title',`addres`='$address',`senfid`='$senfid',`name`='$name',`famili`='$famili',`describ`='$describ',`introduction`='$introduction',`telegram`='$telegram',`instagram`='$instagram',`LinkedIn`='$LinkedIn' $logoq $headerq where `user`='$username'";
$res = mysqli_query($connection, $sql);
if ($res) {
    ...
    while ($fild = mysqli_fetch_assoc($res)) {
        ...
        if (isset($_POST[$item]) == true) {
            $subsenf = $fild['id'];
            $sql2 = "update sub_senf set `senfid`='$subsenf' where `user`='$username' ";
            $res2 = mysqli_query($connection, $sql2);
        }
    }
    msg("ویرایش به درستی صورت گرفت.", "editprofile.php");
}
?>