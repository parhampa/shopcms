    ...
    <script>
        ...
        function morsenf(mors) {
            if (morc > 10) {
                ...
                ?>
                tmmors = mors;
                ajaxcomor("morsenf.php?ty=1&li=" + tmmors <?php if ($famor != "") {
                    echo(' + "' . $famor . '"');
                } ?>, "morplc");
                morc = 0;
                mors = mors + 1;
        ...
        <div class="w3-row box-content">
                ...
                <section>
                    <ul class="sidebar-menu-rtl font-size-1" id="myUL">
                        <input type="text" id="myInput" class="search-asnaf" onkeyup="filterList()"
                               placeholder="صنف خود را جستجو کنید" title="اصناف جهیزیه">
                        <?php
                        $sql = "select * from asnaf where `father`=0 and `type`=1";
            ...
            <div class="w3-col m9 s12">
                <div class="w3-row padding-right-4" id="morplc">
                    ...
                    if (isset($_GET['fa']) == true) {
                        $fa = sqlint($_GET['fa']);
                        $sql = "select * from asnaf where `father`=$fa and `type`=1";
                        $res = mysqli_query($connection, $sql);
                    ...
                    $sql = "select * from `users` where senfid in(select id from asnaf where `type`=1 ) $fatherq limit 0,12";
                    //die($sql);
                    $res = mysqli_query($connection, $sql);
                    while ($fild = mysqli_fetch_assoc($res)) {
                        ?>
