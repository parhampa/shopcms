...
$sql="select * from khadamat where id = $kid and `user`='$user'";
$res=mysqli_query($connection,$sql);
if(mysqli_num_rows($res)<=0)
{
	die("8");
...
if(isset($_POST['fi'])==true)
{
	if($_POST['fi']==0)
	{
		...
		{
			unlink($user."khedmatpic.tmp");
		}
		if(file_exists($user."khedmatpic.jpg")==1)
		{
			unlink($user."khedmatpic.jpg");
		}
	}
}
function base64ToImage($base64_string, $output_file,$user,$kid,$connection) {
	...
	$bs64f=fread($myfile,filesize($user."khedmatpic.tmp"));
	fclose($myfile);
	...
	$sql="update khadamat set `pic`='$pic' where `id`=$kid";
	$res=mysqli_query($connection,$sql);
	echo($pic);
    return $output_file;
}
...
{
	base64ToImage($user."khedmatpic.tmp", $user."khedmatpic.jpg",$user,$kid,$connection);
}
?>